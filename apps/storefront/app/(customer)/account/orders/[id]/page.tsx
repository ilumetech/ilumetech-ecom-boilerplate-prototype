...
import { ArrowLeft, CheckCircle2, Download, HelpCircle, Package, Printer, RefreshCw, Truck } from "lucide-react"
...
const orderData = {
  ...
  items: [
    ...
    { 
      ...
      image: "https://images.unsplash.com/photo-1491933382434-500287f9b54b?w=100&q=80",
      price: "$15.00",
      quantity: 2,
      total: "$30.00",
      status: "Preparing for shipment"
...
const getStatusBadge = (status: string) => {
  switch (status.toLowerCase()) {
    case "delivered":
      return <Badge className="bg-green-100 text-green-800 hover:bg-green-100 dark:bg-green-900/30 dark:text-green-400 border-0 flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5" /> Delivered</Badge>
    case "processing":
      return <Badge className="bg-blue-100 text-blue-800 hover:bg-blue-100 dark:bg-blue-900/30 dark:text-blue-400 border-0 flex items-center gap-1.5"><RefreshCw className="w-3.5 h-3.5 animate-spin-slow" /> Processing</Badge>
    case "shipped":
      return <Badge className="bg-amber-100 text-amber-800 hover:bg-amber-100 dark:bg-amber-900/30 dark:text-amber-400 border-0 flex items-center gap-1.5"><Truck className="w-3.5 h-3.5" /> Shipped</Badge>
    default:
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h2 className="text-3xl font-bold tracking-tight flex items-center gap-3">
              Order #{orderId}
              <span className="hidden sm:inline-block">{getStatusBadge(orderData.status)}</span>
            </h2>
            <p className="text-muted-foreground mt-1 flex items-center gap-2">
              Placed on {orderData.date}
              <span className="sm:hidden ml-2">{getStatusBadge(orderData.status)}</span>
            </p>
          </div>
                    ...
                    <div className="flex-1 flex flex-col sm:flex-row justify-between">
                      ...
                      <div className="mt-4 sm:mt-0 flex flex-col sm:items-end justify-between">
                        ...
                        <div className="flex gap-3 mt-3 sm:mt-0 text-sm">
                          <Link href={`/products/${item.id}`} className="text-primary hover:underline">
                            View Product
                          </Link>
            ...
            <CardContent>
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 bg-muted/30 rounded-lg border border-border/50">
                <div>
                  ...
                  <p className="text-lg tracking-wide font-mono text-primary">{orderData.trackingNumber}</p>
                  <p className="text-sm text-muted-foreground mt-1">Via {orderData.shippingMethod}</p>
                </div>
        ...
        <div className="space-y-6">
          ...
          <Card className="border-border/50 shadow-sm">
            ...
            <CardContent className="p-6 space-y-6">
              <div>
                <h4 className="text-sm font-medium mb-2 text-muted-foreground uppercase tracking-wider">Shipping Address</h4>
              ...
              <div className="border-t border-border/50 pt-6">
                <h4 className="text-sm font-medium mb-2 text-muted-foreground uppercase tracking-wider">Payment Method</h4>
