...
import { Heart, ShoppingCart, Trash2, Search, ArrowRight, Star, ShoppingBag } from "lucide-react"
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
        <div>
          <h2 className="text-3xl font-black uppercase tracking-tight">Wishlist</h2>
      ...
      {items.length > 0 ? (
        filteredItems.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {filteredItems.map((item) => (
              <div key={item.id} className="group relative flex flex-col border border-zinc-200 bg-white transition-all duration-300 hover:border-black hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
                <div className="relative aspect-[4/5] w-full overflow-hidden bg-zinc-100 grayscale hover:grayscale-0 transition-all duration-700">
                  ...
                  {!item.inStock && (
                    <div className="absolute inset-0 bg-white/80 backdrop-blur-[2px] flex items-center justify-center">
                      <span className="text-[10px] font-black uppercase tracking-widest px-4 py-2 bg-black text-white">Out of Stock</span>
                    </div>
                  )}
                ...
                <div className="p-6 flex flex-col gap-4">
                  <div className="flex flex-col gap-1">
                    ...
                    <p className="mt-2 text-sm font-black tracking-tight">{formatPrice(item.price)}</p>
                  </div>
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center border border-dashed border-zinc-200">
            <h3 className="text-xl font-black uppercase tracking-tight mb-2">No results</h3>
            <p className="text-xs font-bold uppercase tracking-widest text-zinc-500 mb-8">
              Try searching for something else.
            </p>
            <Button variant="outline" onClick={() => setSearchQuery("")} className="rounded-none border-black text-[10px] font-bold uppercase tracking-widest">
              Clear Search
            </Button>
          </div>
        )
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center border border-dashed border-zinc-200">
          ...
          <Button asChild className="h-14 rounded-none bg-black px-10 text-[10px] font-black uppercase tracking-[0.2em]">
