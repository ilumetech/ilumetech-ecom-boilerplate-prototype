        ...
        <div className="mb-8 flex flex-col justify-between gap-6 md:flex-row md:items-end">
          <div>
            ...
            <p className="mt-4 max-w-xl text-sm leading-7 text-zinc-600">
              Explore our latest collection of premium sneakers, designed for comfort and built for style. Every pair is 100% original.
            </p>
          </div>
          
          <div className="flex items-center gap-3">
            <Button variant="outline" className="h-10 rounded-none border-zinc-200 text-xs font-bold uppercase tracking-widest hover:border-black">
        ...
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-4">
          ...
          <aside className="hidden lg:block">
            <div className="sticky top-24 space-y-8">
              <FilterGroup title="Categories" items={['Sneakers', 'Running', 'Basketball', 'Training & Gym', 'Lifestyle']} />
              <FilterGroup title="Price" items={['Under Rp1.000.000', 'Rp1.000.000 - Rp2.000.000', 'Over Rp2.000.000']} />
              <FilterGroup title="Color" items={['Black', 'White', 'Blue', 'Red', 'Grey']} />
            </div>
          </aside>
...
function FilterGroup({ title, items }: { title: string; items: string[] }) {
  return (
    <div>
      <h3 className="mb-4 text-xs font-black uppercase tracking-widest text-zinc-900">{title}</h3>
